...
class RealPingWorkerService(
    ...
    private suspend fun startRealPing(guid: String): Pair<Long, String?> {
        val retFailure = -1L
        val configResult = CoreConfigManager.getV2rayConfig4Speedtest(context, guid)
        if (!configResult.status) {
            return Pair(retFailure, null)
        }
        val delay = CoreNativeManager.measureOutboundDelay(configResult.content, SettingsManager.getDelayTestUrl())
        return if (delay >= 0) {
            // Get server address from profile for IP geolocation
            val location = try {
                val profile = MmkvManager.decodeServerConfig(guid)
                val serverAddress = profile?.server
                if (!serverAddress.isNullOrBlank()) {
                    withContext(Dispatchers.IO) {
                        SpeedtestManager.fetchIpLocation(serverAddress)
                    }
                } else null
            } catch (_: Exception) { null }
            Pair(delay, location)
        } else {
            Pair(retFailure, null)
        }
    }
}
